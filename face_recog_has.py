...
def train():
  ...
  path = 'C:/Users/epaysan/Desktop/sumit/py/python/dataset'
  folders = []
  ...
  for f in folders:
    Ids,faces = getImagesWithID(f)
    recognizer.train(faces, np.array(Ids))
    recognizer.save('C:/Users/epaysan/Desktop/sumit/py/trainner.yml')
  cv2.destroyAllWindows()


def my_function():
  directory = str(roll.get())
  print(directory)
  parent_dir = "C:/Users/epaysan/Desktop/sumit/py/python/dataset"
  path = os.path.join(parent_dir, directory)
  try:
    os.mkdir(path)  
  except OSError as error:
    print(error)
  face_cascade=cv2.CascadeClassifier('C:/Users/epaysan/Desktop/sumit/py/haarcascade_frontalface_default.xml')
...
def take_attendance():
  dict={}
  Date= str(date.today())
  fname="C:/Users/epaysan/Desktop/sumit/py/trainner.yml"
    ...
    if cv2.waitKey(1) & 0xFF == ord('q'):
      try:
        for key, value in dict.items() :
          print (type(key))
          myconn = mysql.connector.connect(host="localhost",  user="root",  passwd="root",  database="face_recog",auth_plugin='caching_sha2_password')
          cursor=myconn.cursor()
          cursor.execute("INSERT INTO attendance(date,enroll_id,attendance) VALUES (%s,%s,%s)",(Date,key,"present"))
          myconn.commit()
        break
      except mysql.connector.IntegrityError as e:
        messagebox.showinfo('ALert','Attendance already marked')  
  cap.release()
  cv2.destroyAllWindows()
  buttonoptions()
...
def checkLogin():
  ...
  conn = mysql.connector.connect(host="localhost",  user="root",  passwd="root",  database="face_recog",auth_plugin='caching_sha2_password')
  cur = conn.cursor()
...
def  show_attendance():
  class Database ( wx.Frame ):
    ...
    def btnSearchClick( self, event ):
      ...
      cnn = mysql.connector.connect(host="localhost",  user="root",  passwd="root",  database="face_recog",auth_plugin='caching_sha2_password')
      cur = cnn.cursor()
      cur.execute("SELECT attendance.date,attendance.enroll_id,signup.name,signup.phone_no,signup.guardian_name,signup.guardian_no,attendance.attendance FROM signup,attendance WHERE signup.enroll_id=attendance.enroll_id AND attendance.date LIKE '%"+self.txtSearch.Value+"%' ORDER BY attendance.date DESC")
      rows = cur.fetchall()
...
def new_update():
  def data1():
    global ell
    ell=roll2.get()
    st = mysql.connector.connect(host="localhost",  user="root",  passwd="root",  database="face_recog",auth_plugin='caching_sha2_password')
    student=st.cursor()
  ...
  def data2():
    ...
    d=gno1.get()
    e=sem1.get()
    curr = mysql.connector.connect(host="localhost",  user="root",  passwd="root",  database="face_recog",auth_plugin='caching_sha2_password')
    curr1=curr.cursor()
  ...
  def update2(ell):
    ...
    gno1=IntVar()
    sem1=IntVar()
    my = mysql.connector.connect(host="localhost",  user="root",  passwd="root",  database="face_recog",auth_plugin='caching_sha2_password')
    cn=my.cursor()
...
def show_data():
  ...
  Button(root,text='Capture',width=15,fg='black',command=my_function,bg='light cyan').place(x=180,y=550)
  Button(root, text='Submit',width=15,fg='black',command=database,bg='light cyan').place(x=300,y=550)
  Button(root,text='Back',width=15,fg='black',command=back,bg='light cyan').place(x=420,y=550)
  train()
  root.mainloop()
loginscreen()